...
impl ColrAtom {
    fn new() -> ColrAtom {
        Self {
            size: 0,
            parameter_type: ColorParameterType::Nclc,
            offset: 0,
            primaries: 0,
            transfer_function: 0,
            matrix: 0,
    ...
    fn search(file: &mut File, pattern: &[u8]) -> std::io::Result<Option<Self>> {
        ...
        while reader.read(&mut buffer)? != 0 {
            if buffer.as_slice() == pattern {
                ...
                atom.size += u32::from_be_bytes(size_buf);
        ...
        println!("Pattern {:?} was not found in the file.", hex_pattern);
        Ok(None)
    }
}
...
fn main() {
    ...
    let atom = ColrAtom::search(&mut stream, &COLR_ATOM);
    println!("{:?}", atom);
}